...
-- Справочник пунктов выдачи (пакоматы/пункты)
CREATE TABLE IF NOT EXISTS pickup_points (
    id SERIAL PRIMARY KEY,
    system_point_id VARCHAR(100) NOT NULL UNIQUE,
    provider VARCHAR(20) NOT NULL,
    locker_index VARCHAR(50) NOT NULL,
    name VARCHAR(200) NOT NULL,
    city VARCHAR(100) NOT NULL,
    address VARCHAR(255) NOT NULL,
    postal_code VARCHAR(20) NOT NULL,
    country_code VARCHAR(2) NOT NULL DEFAULT 'LV',
    is_active BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_pickup_points_provider ON pickup_points(provider);
CREATE INDEX IF NOT EXISTS idx_pickup_points_country_code ON pickup_points(country_code);
CREATE INDEX IF NOT EXISTS idx_pickup_points_city ON pickup_points(city);
CREATE INDEX IF NOT EXISTS idx_pickup_points_system_point_id ON pickup_points(system_point_id);

COMMENT ON TABLE pickup_points IS 'Справочник пакоматов и пунктов выдачи для DPD/Omniva';
COMMENT ON COLUMN pickup_points.system_point_id IS 'ID точки в системе службы доставки';
COMMENT ON COLUMN pickup_points.provider IS 'Служба доставки: dpd/omniva';
COMMENT ON COLUMN pickup_points.locker_index IS 'Внутренний индекс/код пакомата';

INSERT INTO pickup_points (system_point_id, provider, locker_index, name, city, address, postal_code, country_code)
VALUES
    ('LV10193', 'dpd', 'DPD-RIX-001', 'DPD Riga Center', 'Riga', 'Brivibas iela 105', 'LV-1001', 'LV'),
    ('LV90008', 'dpd', 'DPD-RIX-002', 'DPD Akropole', 'Riga', 'Maskavas iela 257', 'LV-1019', 'LV'),
    ('LV22017', 'dpd', 'DPD-DGP-001', 'DPD Daugavpils', 'Daugavpils', 'Cietoksna iela 60', 'LV-5401', 'LV'),
    ('EE30001', 'omniva', 'OMN-TLL-001', 'Omniva Tallinn Kesklinn', 'Tallinn', 'Narva mnt 7', '10117', 'EE'),
    ('LT40011', 'omniva', 'OMN-VNO-001', 'Omniva Vilnius Old Town', 'Vilnius', 'Gedimino pr. 9', '01103', 'LT')
ON CONFLICT (system_point_id) DO NOTHING;
...
CREATE OR REPLACE VIEW pickup_points_by_provider AS
SELECT
    provider,
    country_code,
    city,
...
GROUP BY provider, country_code, city
ORDER BY provider, country_code, city;

COMMENT ON VIEW pickup_points_by_provider IS 'Статистика доступных пунктов выдачи по службам доставки';
...
WHERE table_schema = 'public' 
    AND table_name IN ('delivery', 'deliverystatushistory', 'pickup_points')
ORDER BY table_name;
